...
def ad_red_packet(df):
    red_packet_df = df.query('(adId == "1122")', inplace=False)
    # duplicates = df[df.duplicated(subset='adOrderNo', keep=False)]
    logging.info(f'红包页第一个ad的df: \n{red_packet_df.to_string()}')
...
def check_ad_events(row):
    ...
    if row['18'] == row['6'] or row['18'] == row['5']:
        if row['36'] == row['37'] == row['0'] == row['26'] == row['1'] and row['36'] != 0 and row['29'] != 0:
            return "隐藏+点击了adbutton"
        elif row['36'] == row['37'] == row['1'] == row['0'] and row['36'] != 0 and row['29'] != 0:
            return "隐藏+点击"
        elif row['36'] == row['37'] == row['0'] and row['36'] != 0 and row['29'] != 0:
            return "隐藏+展示"
